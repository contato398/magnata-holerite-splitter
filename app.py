...
def _status_funcionario_elegivel(funcionario_id: str):
    """Retorna (elegivel: bool, motivo: Optional[str]). `motivo` é sempre
    um destes 2 valores fixos quando elegivel=False -- nunca contém dado
    pessoal, nome de campo do Airtable ou texto de erro:
    'vinculo_nao_ativo' (status conhecido, mas != Ativo) ou
    'vinculo_indeterminado' (id ausente, falha de rede/HTTP, ou status
