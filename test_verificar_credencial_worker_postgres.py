...
`current_user` (identidade efetiva, que fica `magnata_os` por design
apos o `SET ROLE` automatico do role `magnata_worker_rot2` -- ver
...
def test_prova_completa_quando_banco_e_session_user_conferem():
    # current_user vem "magnata_os" (efetivo, pos SET ROLE automatico) --
    # isso e esperado e nao deve derrubar a prova, so session_user importa.
    abrir_conexao_fn, conexao = _fabrica_conexao(
        [("magnata_os", "magnata_worker_rot2", "magnata_os"), (1,)]
    )
    ...
    assert relatorio["session_user"] == "magnata_worker_rot2"
    assert relatorio["current_user"] == "magnata_os"
...
def test_falha_quando_session_user_e_o_antigo_revogado():
    abrir_conexao_fn, _ = _fabrica_conexao(
        [("magnata_os", "magnata_os", "magnata_os"), (1,)]
    )
...
def test_falha_quando_banco_nao_confere():
    abrir_conexao_fn, _ = _fabrica_conexao(
        [("outro_banco", "magnata_worker_rot2", "magnata_os"), (1,)]
    )
